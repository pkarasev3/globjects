...
int main(int /*argc*/, char * /*argv*/[])
{
    ...
    auto buffer = create_shared<Buffer>();
    ...
    auto query = create_shared<Query>();
    std::cout << "  Query                 : "; info() << query.get();
    auto rbo = create_shared<Renderbuffer>();
    std::cout << "  Renderbuffer          : "; info() << rbo.get();
    auto sampler = create_shared<Sampler>();
    std::cout << "  Sampler               : "; info() << sampler.get();
    auto shader = create_shared<Shader>(GL_VERTEX_SHADER);
    std::cout << "  Shader                : "; info() << shader.get();
    auto texture = create_shared<Texture>();
    std::cout << "  Texture               : "; info() << texture.get();
    auto tf = create_shared<TransformFeedback>();
    std::cout << "  TransformFeedback     : "; info() << tf.get();
    auto vao = create_shared<VertexArray>();
    std::cout << "  VertexArray           : "; info() << vao.get();
    auto uniform = create_shared<Uniform<float>>("Pi", 3.14f);
    ...
    std::vector<std::shared_ptr<Buffer>> buffers{ create_shared<Buffer>(), create_shared<Buffer>() };
